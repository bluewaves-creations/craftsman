...
pub fn run(
    ...
    // NDJSON primary, cucumber-json fallback (module docs). The artifacts
    // are the sole verdict source: cucumber-js exits 1 on red scenarios
    // after writing them.
    ...
    if json_path.is_file() {
        ...
        return Ok(parse_cucumber_json(&text, CucumberJsonDialect::Generic)?);
    }
