...
*(Boundary close-out 2026-07-18 — success line MET: 38/39 scenarios green (the @requires-network self-update stays visible-unknown, see below); release v0.2.0 cut with 11 assets after three tag cuts (two honest failures ledgered: dist resolves its workspace from the invocation root → dist-workspace.toml moved to repo root; a git-mv staged-content slip → verify staged content before tagging); swift-linux canary green on its first live run; installed via the receipt-writing shell installer from an authenticated local mirror (`gh release download` + `CRAFTSMAN_DOWNLOAD_URL=file://…` — the repo is private and GitHub serves private assets only through the API endpoint); doctor 5/5 and `craftsman update` → "0.2.0 is the latest release" from the installed binary with a real receipt. Mid-batch fix commit: cucumber-rs --name filter bypassed the network gate (found live by the paper-landing dogfood; fixed at both selection and harness layers). BLOCKED, human decision needed: the live self-update DOWNLOAD leg cannot complete against a private repo — axoupdater 0.10.0 fetches installers via unauthenticated browser_download_url (lib.rs:483). Options: make the repo public, or wait/patch upstream. Until then the update check leg is fully live-proven and the mirror reinstall is the team update path for majors.)*
